...
/************************************************************************************
  ImageMagick関連の関数定義
************************************************************************************/
class IMResampleImage extends ResampleImage
{
  protected function resample_internal(array $size,string $size_format,string $out_dir,string $out_format,array $pos = ['x' => 0,'y' => 0]) : string
  {
    $width = $size['width'];
    $height = $size['height'];
    ...
    if (true == imagecopyresampled($image, $this->image, 0, 0, $pos['x'], $pos['y'], $width, $height, $src['width'], $src['height']))
    {
