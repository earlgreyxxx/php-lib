...
abstract class Store
{
  /*------------------------------------------------------------------------------
    ストアオブジェクトのシングルトン実装
  ------------------------------------------------------------------------------*/
  public static function GetInstance(string $dsn, string $tablename, string $user = '', string $passwd = '', array $options = []) : static
  {
    static $instances = [];
  ...
  protected ?Action $queryAction = null;
  protected ?Filter $queryFilter = null;
  ...
  ------------------------------------------------------------------------------*/
  public function getQueryAction() : ?Action
  {
    return $this->queryAction;
  }

  public function attachQueryAction(Action $action) : ?Action
  {
    $rv = $this->queryAction;
  ...
  public function getQueryFilter() : ?Filter
  {
    return $this->queryFilter;
  }

  public function attachQueryFilter(Filter $filter) : ?Filter
  {
    $rv = $this->queryFilter;
  ...
  ------------------------------------------------------------------------------*/
  protected function getPlaceholder(string $column) : array
  {
    if (empty(static::$Placeholders))
  ...
  protected function bindValues(PDOStatement $sth, array $columns, array $values, bool $isNamed = false) : PDOStatement
  {
  ...
  //レコードを追加
  protected function _add(array $values, ?array $columns = null) : bool|int
  {
    $rv = false;
    $pdo = $this->dbh;
    if (empty($columns))
    ...
    {
      if (is_callable([$this,'bindValues']))
      {
  ...
  //特定のレコードを更新
  protected function _update(string $column_id, int $id, array $columnvalues) : bool
  {
    $pdo = $this->dbh;
    ...
    if (false !== ($sth = $pdo->prepare($sql)))
    {
      ...
      $this->invoke('modify', ['prepare-sql', $sql, $values]);
    }
  ...
  //特定のレコードを削除
  protected function _remove(string $column_id, int $id): int|false
  {
    $pdo = $this->dbh;
  ...
  protected function fetch(string $query = '', array|string $columns = [] , bool $tr = false) : PDOStatement|false
  {
    $rv = false;
    $pdo = $this->dbh;
  ...
  //セッションから現在ユーザーを得る。
  protected function current_user() : string
  {
  ...
  //総数を表示
  //$query : 文字列
  public function size(string $query = '') : int|false
  {
    $pdo = $this->dbh;
  ...
  // テーブル結合
  public function innerJoin(string $table, string $fk, string $pk = '', string|array $select_columns = '*', string $query = '')
  {
    return $this->join('inner', $table, $fk, $pk, $select_columns, $query);
  }
  public function outerJoin(string $table, string $fk, string $pk = '', string|array $select_columns = '*', string $query = '')
  {
    return $this->join('outer', $table, $fk, $pk, $select_columns, $query);
  }

  protected function join(string $type, string $table, string $fk, string $pk, string|array $select_columns, string $query) : PDOStatement
  {
    $pdo = $this->dbh;
    ...
    $sql = sprintf(
      'SELECT %s FROM %s %s JOIN %s ON %s = %s',
  ...
  public function set_filter(string $type, ?string $query = null) : mixed
  {
    $rv = '';
  ...
  //直近のエラーを取得
  public function lastErrorInfo(mixed $element = 2) : mixed
  {
    $rv = false;
  ...
  //個別カラムの検索・抽出
  //第一引数が配列なら配列を返す
  public function row(mixed $primary_value, string|array $columns, bool $tr = false) : mixed
  {
    $rv = false;
  ...
  public function rowTR(mixed $primary_value, string|array $columns)
  {
  ...
  protected function invoke(string $name, array $params = []) : array|false
  {
    $rv = false;
  ...
  //return   : array | false
  public function search(string $mode,string $word, string $search_column, int $num = 10,int $pos = 0,bool $unicode = false) : array|false
  {
    $pdo = $this->dbh;
  ...
  public function append(array $values, ?array $columns = null) : string|false
  {
  ...
  public function quoteColumns(string|array $columns) : string|array
  {
  ...
  private function end(bool $commit = true) : bool
  {
    return $commit ? $this->dbh->commit() : $this->dbh->rollBack();
  }
  ...
  //最終更新日を得る
  public function getLastUpdate($fmt = 'Y/m/d H:i:s') : string
  {
