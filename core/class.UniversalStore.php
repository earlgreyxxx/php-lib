...
abstract class UniversalStore extends Store
{
  ...
  ******************************************************/
  public static function CreateTable(PDOExtension $pdoex, string $table, ?array $columns, ?array $indexes = null) : void
  {
  ...
  protected string $meta_fkey;

  /*----------------------------------------------------------------------
    form_to_post/post_to_formメソッドは派生先でオーバーライド必須。
  ----------------------------------------------------------------------*/
  protected abstract function form_to_post(?array $form = null): array;
  protected abstract function post_to_form(array $post, string $prefix = ''): array;
  ...
  protected function add(array $post, array $options = []) : string|false
  {
    ...
    try {
      if (false === ($rv = $this->_add($params)))
        throw new RuntimeException(_('Database access error'));
  ...
  /*------------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------------*/
  protected function update(int|string $id, array $post, array $options = []): true|array
  {
    $rv = true;
    ...
    try {
      ...
      if (false === $this->_update($primary_key, $id, $sets))
        throw new RuntimeException(_('failed to store update'));
  ...
  protected function remove(int|string $id, array $options = []) : bool
  {
    ...
    try {
      ...
      if (false === ($rv = $this->_remove($primary_key, $id)))
        throw new RuntimeException(_('failed to store remove'));
  ...
  protected function get(string|array $queries = '', bool $return_statement_handle = false) : array|PDOStatement
  {
    ...
    $sql = sprintf(
      'SELECT DISTINCT %s FROM %s%s LEFT JOIN %s ON %s = %s',
      implode(',', $columns),
    ...
    return !empty($sth) ? $sth->fetchAll(PDO::FETCH_ASSOC) : [];
  }

  protected function count(string $query = '') : int|false
  {
    $pdo = $this->dbh;
  ...
  public function gets(int $num, int $page = 1) : array|PDOStatement
  {
    $offset = ($page - 1) * $num;
    if ($offset < 0)
      $offset = 0;
